...
from SemanticShield import ShieldResult, ShieldConfig, SemanticShield
...
def check_jailbreak(prompt: str, usage_total: int) -> ShieldResult:
    if config.jailbreak.on:
        for model_name in config.jailbreak.models:
            classifier = pipeline('text-classification', model=model_name)
            hits = []
            hits = classifier(prompt)

            if len(hits) > 0:
                for rec in hits:
                    if rec['label'] == 'INJECTION':
                        if rec['score'] > config.jailbreak.threshold:
                            logging.warning(f'Detected prompt injection; score={rec["score"]} threshold={config.jailbreak.threshold}')
                            return ShieldResult(True, fail_type='JAILBREAK', message=config.jailbreak.error, usage=usage_total)
                        else:
                            logging.warning(
                                f'Detected prompt injection below threshold (may warrant manual review); \
                                score={rec["score"]} threshold={config.jailbreak.threshold}'
                            )
            else:
...
for prompt in prompts:
    print('='*80)
    result = check_jailbreak(prompt, 0)
    pprint(result)


# model_name='deepset/deberta-v3-base-injection'
# [{'label': 'LEGIT', 'score': 0.9991480112075806}]
# [{'label': 'INJECTION', 'score': 0.9988642930984497}]
#  "threshold": 0.98
